...
export class HyperStructInfoTracker {
  ...
  genStatusLine (args = {}) {
    ...
    if (!this.struct) {
      ...
      return {status:netCfg}
    }

    var resp = {
      type:'response',
      peers:this.genStatusPeerCount(),
      networkStatus:netCfg,
      speed:this.genStatusNetStats(),
      size:this.size,
      progress:this.genStatusPctDownloaded()
    }
  ...
  genStatusPctDownloaded () {
    if (typeof this.totalBlocks === 'undefined') return '0'
    if (this.totalBlocks) {
      return `${this.totalBlocks ? Math.round(this.downloadedBlocks/this.totalBlocks*100) : 100}`
    }
    return '0'
  }
  ...
  genStatusNetCfg () {
    ...
    if (this.netCfg) {
      if (this.netCfg.announce) netCfg = `Live`
      else if (this.netCfg.lookup) netCfg = `Waiting Seeders`
    }
    return netCfg
  }

  async fetchState (opts = {}) {
    const {wait} = opts;
    this.netCfg = await hyper.getClient().network.status(this.discoveryKey).catch(e => undefined)

    if (!this.struct) {
      if(wait === true){
        await this.attemptLoadStruct();
      }else{
