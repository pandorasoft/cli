...
class HyperStructure extends EventEmitter {
  ...
  async contentDiscoveryKey(){
    const { content, metadata } = await this._getDriveCores(this.key);
    
    return content.discoveryKey.toString('hex');
  }

  _getDriveCores (key) {
    return new Promise((resolve, reject) => {
      this.api.getContent((err, content) => {
        if (err) return reject(err)
        return resolve({ content, metadata: this.api.metadata })
      })
    })
  }
  ...
  async load ({noNetConfig} = {noNetConfig: false}) {
    ...
    if (!this.type || !this.header) {
      ...
      await core.ready()
      if(typeof noNetConfig === 'object'){
        getClient().network.configure(core, {
          lookup: noNetConfig.lookup, 
          announce: noNetConfig.announce
        })
      }else if(typeof noNetConfig === 'boolean'){
