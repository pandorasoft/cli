...
export default {
  ...
  command: async function (args) {
    ...
    // statusLog.print()
    console.log({status:'Accessing network...'});

    var left = await toDftParam(args._[0],args)
    var right = await toDftParam(args._[1],args)
    ...
    var diff = await dft.diff(left, right,{
      compareContent: args.compareContent
    });
    ...
    for (let item of diff) {
      ...
      if (item.change === 'add') {
        console.log({change:chalk.green(`${chalk.bold('Add:')} ${path}`)})
      } else if (item.change === 'del') {
        console.log({change:chalk.red(`${chalk.bold('Delete:')} ${path}`)})
      } else {
        console.log({change:`${chalk.bold('Change:')} ${path}`})
      }
    }
    if (diff.length === 0) {
      console.log({status:'No differences found.'});
    }
...
async function toDftParam (str,args) {
  if (isUrl(str)) {
