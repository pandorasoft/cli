...
const SYNC_INTERVAL_WRITE = 1000;
const SYNC_INTERVAL_LOG = 1000;
const FULL_USAGE = `
Options:
...
export default {
  ...
  options: [
    ...
    {name: 'compareContent', default: false, boolean: true},
    {name: 'syncIntervalWrite', default: SYNC_INTERVAL_WRITE, boolean: false},
    {name: 'syncIntervalLog', default: SYNC_INTERVAL_LOG, boolean: false},
    {name: 'lookup', default: true, boolean: true},
    {name: 'announce', default: true, boolean: true},
    {name: 'watch', abbr: 'w', default: false, boolean: true},
    {name: 'op', default: 'scan', boolean: false},
  ],
  command: async function (args) {
    ...
    try{
      ...
      var leftArgs = await parseArgs(args._[0],args)
      var rightArgs = args._[1] ? await parseArgs(args._[1],args) : await createTarget(leftArgs)
      // if (!args._[1]) console.log('Creating new hyperdrive...')
      // console.log(chalk.bold(`Source: ${leftArgs.raw}`))
      // console.log(chalk.bold(`Target: ${rightArgs.raw}`))
      ...
      // console.log(live ? 'Live syncing (Ctrl+c to exit)...' : 'Syncing...')
  
      var statusLines = null;//['']
      var statusLog = null;//statusLogger(statusLines)
      if (leftArgs.isHyper) await setupTracker(leftArgs.fs.key.toString('hex'), statusLines, statusLog,args)
      if (rightArgs.isHyper) await setupTracker(rightArgs.fs.key.toString('hex'), statusLines, statusLog,args)
      ...
      const watcher = watch(left, debounce(() => sync(left, right, args, {statusLines, statusLog}), args.syncIntervalWrite))
      let exiting = false
      ...
      if(err.message == 'Block not available from peers'){
        console.log(JSON.stringify({type:'response',status:`${args.op}_error`}));
      }
      throw err;
    }
  }
...
async function sync (left, right, opts = {}, {statusLines, statusLog}) {
  ...
  // statusLog.print(statusLines)
  console.log(JSON.stringify({type:'response',status:'compare'}));
  ...
  if (diff.length !== 0) {
    console.log(JSON.stringify({type:'response',status:opts.op}));
    await new Promise(resolve => {
      ...
      s.on('data', action => {
        ...
        // statusLog.print(statusLines)
        if(opts.op == 'scan'){
          console.log(JSON.stringify({type:'response',subType:'scan',progress:pct}));
          // console.log(JSON.stringify({type:'response',progress:pct,event:`${op} ${action.path}`}));
        }
      })
      s.on('error', err => {
        console.log(JSON.stringify({type:'response',status:`${opts.op}_error`}));
  ...
  // statusLog.print(statusLines)
  console.log(JSON.stringify({type:'response',status:'ready'}));
}
...
async function parseArgs (str,args) {
  if (isUrl(str)) {
    let urlp = parseHyperUrl(str)
    let drive = await HyperStruct.get(urlp.hostname, {expect: 'hyperdrive',noNetConfig:{
      lookup:args.lookup,
...
async function setupTracker (key, statusLines, statusLog,args) {
  ...
  const output = () => {
    ...
    // statusLog.print(statusLines)
    console.log(JSON.stringify(tracker.genStatusLine(args)));
  }
  setInterval(output, args.syncIntervalLog).unref()

  // periodically calculate the size of the hyper structure
  var firstRun = true
  const updateState = async () => {
    await tracker.fetchState().catch((err)=>{
      console.error(JSON.stringify({event:err.toString()}));
    })
    if (firstRun) {
      output()
    ...
    setTimeout(updateState, args.syncIntervalLog).unref()
  }
  await updateState()
}