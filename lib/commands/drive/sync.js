...
import {xtendpa} from '../../helper.js';

const INTERVAL_WRITE = 1000;
...
const OPTIONS = [
  ...
  {name: 'intervalWrite', default: INTERVAL_WRITE, boolean: false},
...
export default {
  ...
  command: async function (args) {
    ...
    try{
      ...
      const watcher = watch(left, debounce(() => sync(left, right, args, {statusLines, statusLog}), args.intervalWrite))
      let exiting = false
  ...
  if (diff.length !== 0) {
    ...
    await new Promise(resolve => {
      ...
      s.on('data', action => {
        ...
        // statusLog.print(statusLines)
        if(opts.op == 'scan'){
          console.log(JSON.stringify({type:'response',subType:'scan',progressScan:pct}));
...
async function setupTracker (key, statusLines, statusLog,args) {
  ...
  setInterval(output, args.intervalLog).unref()
  ...
  const updateState = async () => {
    ...
    setTimeout(updateState, args.intervalLog).unref()
  }
  await updateState()
}