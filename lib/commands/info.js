...
export default {
  ...
  command: async function (args) {
    console.log(JSON.stringify({event:'start network logger'}));
    xtendpa(args,{
      op:String,
      intervalLog:Number,
    ...
    const start = async()=>{
      ...
      // console.log(JSON.stringify({event:keys,data:data}));
      keys.map(key=>{
        if(response[key]){
      ...
      if (!keys.length) {
        // if(data.length){
        //   console.log(JSON.stringify({type:'restart'}));
      ...
      await Promise.all(keys.map(async (key, i) => {
        ...
        const updateStatusLine = () => {
          if (!network && !mirror) {
            ...
            if(!response[key]){
              //jika sudah dihapus oleh client, maka:
              mirroringClient.unmirror(key, tracker.struct.type);
            }else{
        ...
        const updateState = async () => {
          try {
            ...
            if (e.toString().includes('RPC stream destroyed')) {
              console.log(JSON.stringify({event:e.toString(),data:response[key]}));
              // ignore
            } else {
