...
export default {
  ...
  command: async function (args) {
    ...
    if (args._[0] === 'drive' || args._[0] === 'hyperdrive') {
      struct = await HyperStruct.create('hyperdrive',{noNetConfig:false})
      console.log(JSON.stringify({type:'response',size:0,keyStr:struct.keyStr,shareKey:struct.url,discoveryKey:struct.discoveryKey.toString('hex'),contentDiscoveryKey:await struct.contentDiscoveryKey()}));
    ...
    console.log(JSON.stringify({event:`Seeding.`}));
